    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Identify constraints and capacity issues</p>
        </div>
        <Link
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h4 className="font-semibold text-yellow-900 mb-2">Bottleneck Detection Logic</h4>
          ...
          {stations.map((station, index) => (
            <div key={station.id} className="flex items-center gap-4">
              <div className={`
                ...
                ${station.status === 'bottleneck' ? 'shadow-lg scale-105 ring-4 ring-red-100' : ''}
              `}>
                <div className="text-center">
                  <div className="font-semibold text-sm mb-2 opacity-80">{station.id}</div>
                  <div className="text-sm font-bold mb-3">{station.name}</div>
                  <div className="text-2xl font-bold mb-1">{station.avgTaskTime}m</div>
                  <div className="text-xs mb-2 opacity-80">Avg Time</div>
              ...
              {index < stations.length - 1 && (
                <div className="flex items-center opacity-50">
                  ...
                  <div className="w-0 h-0 border-t-4 border-t-transparent border-b-4 border-b-transparent border-l-6 border-l-gray-400"></div>
                </div>
              )}
            </div>
          ))}
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-white border-b border-gray-200">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Station ID</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Name</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Avg Task Time</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Workload (min)</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Utilization</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Queue Pressure</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {stations.map((station) => (
                <tr 
                  ...
                  `}
                >
                  <td className="px-6 py-4 text-sm font-semibold text-gray-900">{station.id}</td>
                  <td className="px-6 py-4 text-sm font-medium text-gray-700">{station.name}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">{station.avgTaskTime}m</td>
                  <td className="px-6 py-4 text-sm text-gray-600">{station.workload}m</td>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <div className="bg-white/60 rounded p-3 text-sm text-red-900 space-y-2 border border-red-100">
                <p className="flex justify-between"><span>Workload:</span> <strong>2160 mins</strong></p>
                <p className="flex justify-between"><span>Avg Task Time:</span> <strong>12 mins/order</strong></p>
                <p className="flex justify-between"><span>Queue Pressure:</span> <strong>High (Backing up)</strong></p>
